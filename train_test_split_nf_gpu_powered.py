...
import bprnn_netflix_alpha_gpu_powered
import imp
...
imp.reload(bprnn_netflix_alpha_gpu_powered)
...
model = bprnn_netflix_alpha_gpu_powered.BPR_NN(N_USERS, N_ITEMS, N_EMBEDDINGS, alpha=float(sys.argv[2]), beta=float(sys.argv[3]), alpha_reg=float(sys.argv[5]), inner_net=inner_network)
model.build_graph()
