...
class TripletsDataset(object):
    ...
    """
    def __init__(self, raw_data_train, raw_data_test, binarize = 1, threshold_user=20, threshold_item=0, rnd_seed=42):

        self.threshold_user = threshold_user
        self.threshold_item = threshold_item
        self.train_len = len(raw_data_train)
        ...
        for row in tqdm(raw_data, desc='Assemble .data', leave=False):
            ...
            self.data[u] = self.data.get(u, []) + [(i, r, ind)]
    ...
    def train_test_split(self, seed=None):
        ...
        n_train = np.copy(self.train_len)
        ...
        for u in tqdm(self.data, desc='Split users', leave=False):
            ...
            train_inds = set(range(self.train_len))
           
            for n, (i, r, ind) in enumerate(self.data[u]):
    ...
    def sample_train_batch(self,n_samples=256):
        ...
        for i in range(n_samples):
            result = self.sample_train_triple()
