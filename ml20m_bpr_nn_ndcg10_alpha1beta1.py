...
N_ITEMS = 131263 #int(max(raw_data[:, 1])) + 1
N_EMBEDDINGS = 1
...
model = bprnn.BPR_NN(N_USERS, N_ITEMS, N_EMBEDDINGS, alpha=1.0, beta=1.0, alpha_reg=0.0, inner_net=inner_network)
model.build_graph()
...
export_basename = '/data/sidana/nnmf_ranking/ml20m/vectors/'
export_pred = open(export_basename + 'pr_10_11', 'w')
export_true = open(export_basename + 'gt_10_11', 'w')
...
for u in tqdm(ds.data_keys, desc='Prediction', leave=True):
    ...
    itemsGroundTruth = np.array([i for (i,r) in ds.test[u] if r == 2])
