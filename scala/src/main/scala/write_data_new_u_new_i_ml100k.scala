...
object write_data_new_u_new_i_ml100k {
...
val newUsers = sqlContext.read
    ...
    .option("inferSchema", "true") // Automatically infer data types
    .option("delimiter", "\t")
    .load("/data/sidana/recnet_draft/cold_start/data/ml100k/dat.ml100k.users.new")
...
val newItems = sqlContext.read
    ...
    .option("inferSchema", "true") // Automatically infer data types
    .option("delimiter", "\t")
    .load("/data/sidana/recnet_draft/cold_start/data/ml100k/dat.ml100k.items.new")
...
trainUsersItems.coalesce(1,false).saveAsTextFile("/data/sidana/recnet_draft/cold_start/data/ml100k/dat.ml100k.newusersnewitems")
