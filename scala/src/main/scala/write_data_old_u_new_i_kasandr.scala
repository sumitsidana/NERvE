...
object write_data_old_u_new_i_kasandr {
...
val oldUsers = sqlContext.read
    ...
    .option("inferSchema", "true") // Automatically infer data types
    .option("delimiter", "\t")
    .load("/data/sidana/recnet_draft/cold_start/data/kasandr/dat.kasandr.users.old")
...
trainUsersItems.coalesce(1,false).saveAsTextFile("/data/sidana/recnet_draft/cold_start/data/kasandr/dat.kasandr.oldusersnewitems")
	}
}
