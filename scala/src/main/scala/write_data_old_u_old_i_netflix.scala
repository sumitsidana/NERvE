...
object write_data_old_u_old_i_netflix {
...
val oldItems = sqlContext.read
    ...
    .option("inferSchema", "true") // Automatically infer data types
    .option("delimiter", "\t")
    .load("/data/sidana/recnet_draft/cold_start/data/netflix/dat.netflix.items.old")
...
filetobewritten.coalesce(1,false).saveAsTextFile("/data/sidana/recnet_draft/cold_start/data/netflix/dat.netflix.oldusersolditems")
	}
}
