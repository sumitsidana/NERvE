...
import bprnn_kasandr_param_tune_all
import imp
...
imp.reload(bprnn_kasandr_param_tune_all)
...
model = bprnn_kasandr_param_tune_all.BPR_NN(N_USERS, N_ITEMS, N_EMBEDDINGS, alpha=int(sys.argv[2]), beta=int(sys.argv[3]), alpha_reg=float(sys.argv[5]), inner_net=inner_network)
model.build_graph()
